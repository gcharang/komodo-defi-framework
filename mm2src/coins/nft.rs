...
use crate::nft::nft_structs::{build_nft_with_empty_meta, BuildNftFields, MnemonicHQRes, NftCommon, NftCtx,
                              NftTransferCommon, RefreshMetadataReq, SpamContractReq, SpamContractRes, TransferMeta,
...
async fn get_moralis_metadata(
    ...
    let nft_metadata = build_nft_from_moralis(*chain, nft_moralis, contract_type, url_antispam).await;
    Ok(nft_metadata)
}
...
fn construct_camo_url_with_token(token_uri: &str, url_antispam: &Url) -> Option<Url> {
    ...
    if let Ok(mut segments) = url.path_segments_mut() {
        segments.push(hex_token_uri.as_str());
    } else {
        return None;
    }
    Some(url)
        ...
        .await?
    {
        ...
        None => {
            ...
            .await
            {
                ...
                Err(_) => build_nft_with_empty_meta(BuildNftFields {
    ...
    T: NftListStorageOps + NftTransferHistoryStorageOps,
{
    ...
    for addr_id_pair in nft_token_addr_id.into_iter() {
        if let Ok(nft_meta) = get_moralis_metadata(
