...
mod tests {
    ...
    fn block_template_request_mode_serialize() {
        assert_eq!(
            serde_json::to_string(&BlockTemplateRequestMode::Template).unwrap(),
            r#""template""#
        );
        assert_eq!(
            serde_json::to_string(&BlockTemplateRequestMode::Proposal).unwrap(),
            r#""proposal""#
        );
    }
    ...
    fn block_template_request_deserialize() {
        ...
        assert_eq!(
            serde_json::from_str::<BlockTemplateRequest>(r#"{"mode":"template","capabilities":["a"],"rules":["b"]}"#)
                .unwrap(),
            BlockTemplateRequest {
                mode: Some(BlockTemplateRequestMode::Template),
                capabilities: Some(vec!["a".to_owned()].into_iter().collect()),
